...
var defaultNetwork = Networks.defaultNetwork;
var defaultPath = '/insight-tealcoin-api';
...
function Insight(url, network) {
  ...
  if (Networks.get(url)) {
    ...
    if (network === Networks.livenet) {
      url = 'http://explorer.firstep-it.net:3001';
	  defaultPath = '/insight-tealcoin-api';
    } else if (network === Networks.testnet) {
      url = 'http://explorer.firstep-it.net:3002';
	  defaultPath = '/insight-tealcoin-api';
    } else if (network === Networks.bitcoin) {
      url = 'https://insight.bitpay.com';
	  defaultPath = '/api';
    } else if (network === Networks.bitcoin_testnet) {
...
Insight.prototype.getTransaction = function(txid, callback) {
  ...
  this.requestGet(defaultPath+'/tx/' + txid, function(err, res, body) {
...
Insight.prototype.getUtxos = function(addresses, callback) {
  ...
  addresses = _.map(addresses, function(address) {
    return new Address(address, defaultNetwork);
  });

  this.requestPost(defaultPath+'/addrs/utxo', {
...
Insight.prototype.broadcast = function(transaction, callback) {
  ...
  this.requestPost(defaultPath+'/tx/send', {
    rawtx: transaction
...
Insight.prototype.address = function(address, callback) {
  $.checkArgument(_.isFunction(callback));
  address = new Address(address, defaultNetwork);

  this.requestGet(defaultPath+'/addr/' + address.toString(), function(err, res, body) {
    if (err || res.statusCode !== 200) {
      return callback(err || body);
    }
    var info;
    try {
      info = AddressInfo.fromInsight(body, defaultNetwork);
